...
// 导入用户操作监控注入函数
const { injectUserActionMonitor } = require("./utils/inject-monitor");
  ...
  try {
    ...
    try {
      ...
      if (!guideBrowser) {
        ...
        guideContext = await guideBrowser
          .newContext({
            javaScriptEnabled: true,
            ignoreHTTPSErrors: true,
            // 修复：viewport.width 和 height 必须是整数，不能是字符串
      ...
      if (isAutoRecord) {
        ...
        try {
          ...
          // 修复：使用正确的 sendToClient 方法，而不是不存在的 send 方法
          ws.sendToClient({ type: "ws-status", status: "connected" });
        } catch (err) {
          console.error("Failed to bind communication:", err);
          throw new Error("failed to bind communication: " + err.message);
        }

        // 开启全页面操作监听（点击/输入/选择/滚动/按键）
        try {
          await injectUserActionMonitor(guidePage);
          // 修复：使用正确的 sendToClient 方法
          ws.sendToClient({ type: "user-action-monitor-status", status: "started" });
        } catch (err) {
          console.error("Failed to inject user action monitor:", err);
          throw new Error("failed to inject user action monitor: " + err.message);
        }
      }
      ...
      return res.json(
        createResponse(
          ...
          200,
          isAutoRecord ? "guide page opened with auto recording" : "guide page opened",
        ),
      );
    } catch (err) {
...
app.post("/start-record", async (req, res) => {
    ...
    if (clearExisting === undefined) {
      return res.status(409).json(
        createResponse(
          ...
          409,
          "existing recordings found for this host",
        ),
      );
    }
  ...
  child.stdout.on("data", (chunk) => {
    ...
    s.split(/\r?\n/).forEach((l) => {
      if (l) logs.push({ ts: new Date().toISOString(), line: l });
    });
